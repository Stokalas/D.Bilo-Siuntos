...
namespace Infrastructure.DataAccess
{
    public class ParcelService : IParcelService
    {
        ...
        public async Task<IList<Parcel>> GetAllByUserId(int shipperId)
        {
            try
            {
                return await _dbContext.Parcels.Include(c => c.ShippingAddress).Include(t => t.DeliveryAddress).Include(s => s.Status).Include(v => v.Terminal).Where(x => x.Shipper.Id == shipperId).ToListAsync();
            }
            catch
            {
                //Log error
